...
export async function register(req, res) {
    ...
    let isValid = await validate_fields(name, email);
    ...
    if (isValid && (!existingname||existingname.length == 0)){
        let new_player = new Users(name, email, password, isHost);
...
export async function update_player(req, res) {
    ...
    let email = req.body.email;
    let isValid = await validate_fields(new_name, email);
    if (isValid){
        let msg = await Users.update(name_to_match, new Users(new_name, email))
        res.send(msg);
    } else {
...
export async function buyStock(req, res) {
    ...
    if (player[0].isHost){
        res.status(404).send(`User ${playerName} is not a player(switch to player).`);
    }
...
export async function user_change(req,res){
    ...
    try {
        ...
        player[0].toggleUserRole();

        // Optionally, you can save the updated user data to the database here.

        res.status(200).send(`User role changed successfully for ${playerName}.`);
    } catch (error) {
...
//     try{
//         let info = Users.get_player(name);
